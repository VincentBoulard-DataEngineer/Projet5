...
6. **Logging**
    ...
    - Processus journalisé dans `logs/migration.log`.
    - Logs visibles en live dans la console.

7. **Tests Pytest** (optionnel)
...
## Sécurité & gestion des accès MongoDB (RBAC)

Trois rôles sont envisagés afin de séparer clairement les responsabilités et limiter les accès au strict nécessaire.
...
## Vérification rapide dans MongoDB

    docker compose exec <nom_du_service_mongodb> mongosh
    use healthcare_db
    show collections
...
Depuis le conteneur
    ...
    docker compose exec migration cat /logs/migration.log
